...
class President extends Connection{
	...
	public function insert(){
		...
		if(empty($this->image)){
			...
			if($query->execute()){
				...
				echo "<script>window.location='president_speech.php'</script>";
			}
		}
		else{
	...
	public function delete(){
		...
		$query->bindValue(':status',12, PDO::PARAM_STR);	
		
		if($query->execute()){
			$_SESSION['message'] = "You Have Deleted Information";
			echo "<script>window.open('president_speech.php','_self')</script>";
		}
	}
